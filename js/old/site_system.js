...
var workflowgroup_filter = ["INFORM_SAHEL"]; //["INFORM_EAST_AFRICA", "INFORM_GTM", "INFORM_LAC", "INFORM_LBN", "INFORM_SAHEL", "INFORM_CCA", "INFORM_COL"];

//////////////////////
// DEFINE VARIABLES //
//////////////////////

var api_base = 'http://www.inform-index.org/API/InformAPI/';
...
load_dashboard = function(inform_model,workflow_id) {
    ...
    d3.json(systems,function(data_systems) {
        ...
        lookup_systems_fullname = genLookup_systems('Fullname');
        ...
        for (var i=0;i<data_systems.length;i++) {
            ...
            //if (data_systems[i].Code !== 'INFORM') {
                ...
                eval("var ul" + data_systems[i].Code +" = document.createElement('ul');");
                eval("ul" + data_systems[i].Code + ".setAttribute('class','dropdown-menu');");
                eval("ul" + data_systems[i].Code + ".setAttribute('id','system-" + data_systems[i].Code + "');");
                eval("li.appendChild(ul" + data_systems[i].Code + ");");
        ...
        d3.json(setting == 'api' ? workflows : 'data/workflows.json',function(data_workflows) {
            ...
            for (var i=0;i<data_workflows.length;i++) {
                if (data_workflows[i].indexOf('_') <= -1) {
                    var year = data_workflows[i].substring(6,10)*1;
            ...
            // for (var i=0;i<data_workflows.length;i++) {
                ...
                //if (workflowgroup_filter.indexOf(data_workflows[i]) > -1) {
                    ...
                    // a.setAttribute('data-toggle','dropdown');
                    // a.setAttribute('href','#');
                    //a.setAttribute('id',data_workflows[i]);
                    ...
                    //var workflow_info = api_base + 'workflows/GetByWorkflowGroup/' + data_workflows[i];
                    ...
                    (function (_i) {
                        ...
                        d3.json(setting == 'api' ? workflow_info : 'data/workflow' + workflow_string + '.json',function(workflow_info) {
                            ...
                            for (var j=0;j<workflow_info.length;j++) {
                                //if (workflow_info[j].Author == 'anonymous') {
                                if (workflow_info[j].FlagGnaPublished !== null) {

                                    // var a_system = document.getElementById(data_workflows[_i]);
                                    // if (workflow_info[j].System.toUpperCase() !== 'INFORM') {
                                        // if (a_system.innerHTML !== lookup_systems_fullname[workflow_info[j].System]) {
                                            // a_system.innerHTML = lookup_systems_fullname[workflow_info[j].System] + '<i class="fa fa-angle-left export-btn-arrow menu-arrow"></i>';
                                        // } 
                                    // } else {
                                        // a_system.innerHTML = data_workflows[_i] + '<i class="fa fa-angle-left export-btn-arrow menu-arrow"></i>';
                                    // }
                                    var system = workflow_info[j].System.toUpperCase();
                                    if (system == data_systems[_i].Code && system !== 'INFORM') {
                                        
                                        var ul = document.getElementById('system-'+system);
                                        var li = document.createElement('li');
                                        eval("ul" + system + ".appendChild(li);");
                                        //ul.appendChild(li);
                                        var a = document.createElement('a');
                                        ...
                                        if (d.workflow_id == workflow_info[j].WorkflowId) {d.workflow_name = workflow_info[j].Name;};
                                    }                                    
    ...
    //$('.dropdown a.models0').next('ul').css('display','none');
    //if (document.getElementsByClassName('models0')[0].getAttribute('aria-expanded') == 'true') {$('.dropdown a.models0').next('ul').toggle();};
    ...
    d3.json(default_api,function(default_workflow) {
        ...
        d3.json(setting == 'api' ? workflow_api : 'data/workflow' + workflow_string + '.json',function(workflow_info) {
            ...
            //Get source-indicators for metadata
            ...
            d3.json(setting == 'api' ? source_api : 'data/sourcedata.json',function(source_data) {
                ...
                d3.json(setting == 'api' ? url_meta : 'data/metadata_' + country_code + '.json', function(meta_data) {
                    ...
                    d3.json(setting == 'api' ? url_data : 'data/inform_data_' + country_code + '.json', function(inform_data){
                        ...
                        //Load Geodata
                        //var url_geo = api_base + 'Countries/GeometryLocation/?modelName=' + d.system;
                        //geo_data_url = 'countries_' + country_code;
                        //if (d.workflow_id == 355) {geo_data_url = 'countries_SAHEL_02';};
                        ...
                        d3.json(url_geo_direct, function(geo_data) {
...
var generateCharts = function (d){
    ...
    //$(document).ready(function(){
        /* $('.dropdown a.models0').on("click", function(e){
            if (document.getElementsByClassName('models0')[0].getAttribute('aria-expanded') == 'false') {
                console.log('hallo');
                //$('.dropdown a.models0').next('ul').css('display','none');
                $('.dropdown-submenu a.models1').next('ul').css('display','none');
                $('.dropdown-submenu a.models2').next('ul').css('display','none');
        ...
        $('.dropdown-submenu a.models2').on("click", function(e){
            $('.dropdown-submenu a.models2').not(this).next('ul').css('display','none');
            $(this).next('ul').toggle();
            e.stopPropagation();
            e.preventDefault();
        });
